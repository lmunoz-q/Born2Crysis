...
/*   Updated: 2019/07/06 20:40:17 by mfischer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "world.h"

static void		fill_buffer(double *buff[4], t_polygon *p, int n)
{
	int			i;
	int			j;
...
static void		compute_angles(double *buff[4], int nb, double normal[3])
{
	int		i;
	double	ref[3];
	double	tmp[3];
...
static void		gen_outbound(double **buffer, int n, t_mesh *m)
{
	...
	if (!(m->portal_outbound = (double **)malloc(sizeof(double [4]) * (points->top + 1))))
		return ;
