...
/*   Updated: 2019/08/31 22:44:39 by tfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_editor_interface *editor_interface)
{
	...
	if (!libui_create_label(&editor_interface->slider_title_alpha,
		(SDL_Rect){165, 618, 160, 20}, "Alpha", editor_interface->font))
		return (-1);
	...
	if (!libui_create_slider(&editor_interface->slider_alpha,
		(SDL_Rect){183, 640, 275, 20}, SDL_FALSE))
		return (-1);
	...
	if (!libui_create_label(&editor_interface->label_alpha,
		(SDL_Rect){165, 640, 16, 20}, "a:", editor_interface->font))
		return (-1);
	if (!libui_create_label(&editor_interface->label_nb_alpha,
		(SDL_Rect){460, 640, 40, 20}, "0", editor_interface->font))
		return (-1);
