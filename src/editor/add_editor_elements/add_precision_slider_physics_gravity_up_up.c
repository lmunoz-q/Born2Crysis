...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   add_precision_slider_physics_gravity_up_up.        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tfernand <tfernand@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/08/31 21:49:51 by tfernand          #+#    #+#             */
/*   Updated: 2019/09/01 00:37:35 by tfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "editor.h"

static int	increase_dot01_gravity_x(SDL_Event *event,
...
static int	increase_dot01_gravity_y(SDL_Event *event,
	t_libui_widget *widget,
	void *data)
{
	double						tmp_x;
...
static int	increase_dot01_gravity_z(SDL_Event *event,
	t_libui_widget *widget,
	void *data)
{
	double						tmp_x;
	...
	t_editor_interface *editor_interface)
{
	...
	cons = (t_libui_textbutton_constructor){ws,
		&editor_interface->editor_container, {220, 700, 50, 20}, {5, 2, 50,
		50}, "+0.01", editor_interface->font, cons.theme};
	if (libui_create_textbutton(&(editor_interface->
		up_button_physics_gravity_x), &cons))
		return (1);
	libui_callback_setpressed(&(editor_interface->up_button_physics_gravity_x),
		increase_dot01_gravity_x, SDL_MOUSEBUTTONDOWN, editor_interface);
	cons.rect = (SDL_Rect){.x = 220, .y = 722, .w = 50, .h = 20};
	if (libui_create_textbutton(&(editor_interface->
		up_button_physics_gravity_y), &cons))
		return (1);
	libui_callback_setpressed(&(editor_interface->up_button_physics_gravity_y),
		increase_dot01_gravity_y, SDL_MOUSEBUTTONDOWN, editor_interface);
	cons.rect = (SDL_Rect){.x = 220, .y = 744, .w = 50, .h = 20};
	if (libui_create_textbutton(&(editor_interface->
		up_button_physics_gravity_z), &cons))
		return (1);
	libui_callback_setpressed(&(editor_interface->up_button_physics_gravity_z),
		increase_dot01_gravity_z, SDL_MOUSEBUTTONDOWN, editor_interface);
	return (0);
}
