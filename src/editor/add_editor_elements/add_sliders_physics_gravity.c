...
/*   Updated: 2019/08/31 16:19:57 by tfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <doom_nukem.h>

static int	add_s_p_g_sub1(t_editor_interface *editor_interface)
{
	...
	if (!libui_create_label(&editor_interface->label_physics_gravity_x,
		...
		return (-1);
	if (!libui_create_label(&editor_interface->label_nb_physics_gravity_x,
		(SDL_Rect){122, 700, 40, 20}, "0", editor_interface->font))
		return (-1);
	dvs_x.value = &(editor_interface->sector_gravity.n.x);
	dvs_x.label = &editor_interface->label_nb_physics_gravity_x;
...
static int	add_s_p_g_sub2(t_editor_interface *editor_interface)
{
	...
	dvs_y.label = &editor_interface->label_nb_physics_gravity_y;
	libui_progressbar_set_minmax_value(
	...
	if (!libui_create_label(&editor_interface->label_physics_gravity_y,
		...
		return (-1);
	if (!libui_create_label(&editor_interface->label_nb_physics_gravity_y,
...
static int	add_s_p_g_sub3(t_editor_interface *editor_interface)
{
	...
	if (!libui_create_label(&editor_interface->label_physics_gravity_z,
		...
		return (-1);
	if (!libui_create_label(&editor_interface->label_nb_physics_gravity_z,
		(SDL_Rect){122, 744, 40, 20}, "0", editor_interface->font))
		return (-1);
	dvs_z.value = &(editor_interface->sector_gravity.n.z);
	dvs_z.label = &editor_interface->label_nb_physics_gravity_z;
	...
	t_editor_interface *editor_interface)
{
	...
	libui_widgets_add_widget(ws, &editor_interface->label_nb_physics_gravity_x,
		0, &editor_interface->editor_container);
	...
	libui_widgets_add_widget(ws, &editor_interface->label_physics_gravity_y, 0,
		&editor_interface->editor_container);
	libui_widgets_add_widget(ws, &editor_interface->label_nb_physics_gravity_y,
		0, &editor_interface->editor_container);
	...
	libui_widgets_add_widget(ws, &editor_interface->label_physics_gravity_z, 0,
		&editor_interface->editor_container);
	libui_widgets_add_widget(ws, &editor_interface->label_nb_physics_gravity_z,
		0, &editor_interface->editor_container);
