...
void 	render_editor_view(t_world *world, t_editor_interface *ei)
{
	...
	openworld_render(world, &ei->editor_cam, ei->view_container.texture, ei->secteur_courant);
	if (ei->secteur_courant != ei->secteur2_courant)
		openworld_render(world, &ei->editor_cam, ei->view_container.texture, ei->secteur2_courant);
