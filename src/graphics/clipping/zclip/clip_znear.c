...
static void		clip_1out2in(t_clipper *c, t_polygon *p, t_polygon *o)
{
	...
	ratio = ((t_edge *)c->outside->data[0])->dist
		/ (((t_edge *)c->inside->data[0])->p->a[2]
		- ((t_edge *)c->outside->data[0])->p->a[2]);
	r2 = ((t_edge *)c->outside->data[0])->dist
		/ (((t_edge *)c->inside->data[1])->p->a[2]
		- ((t_edge *)c->outside->data[0])->p->a[2]);
	i_clip_1out2in(c, o);
	...
	((t_edge *)c->outside->data[0])->l[0] += (((t_edge *)
		c->inside->data[1])->l[0] - ((t_edge *)c->outside->data[0])->l[0]) * r2;
}
...
int				clip_znear(t_polygon *p, int count)
{
	...
	while (++i < count)
		if (!(p[i].tex_id == -1 || !(c = init_clipper())))
		{
			...
			stack_push((e[0].p->a[2] >= ZNEAR) ? c->inside : c->outside, &e[0]);
			stack_push((e[1].p->a[2] >= ZNEAR) ? c->inside : c->outside, &e[1]);
			stack_push((e[2].p->a[2] >= ZNEAR) ? c->inside : c->outside, &e[2]);
			if (c->outside->top == 1)
				clip_2out1in_z(c);
			if (c->outside->top == 0)
