...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/03 17:40:35 by hmartzol          #+#    #+#             */
/*   Updated: 2019/09/03 17:40:36 by hmartzol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <bmml_functions.h>

static inline int	i_more(t_ptr_type type, t_data val1, t_data val2)
{
	if (type == PT_I8)
...
t_error_type		process_more(t_processor *p, t_op_param *param)
{
	...
	if (get_casted_parameter(p, &param[2], &val2, type1) != ET_OK)
		return (p->error);
	i_set(ptr, type2, i_more(type1, val1, val2));
	return (p->error);
}
